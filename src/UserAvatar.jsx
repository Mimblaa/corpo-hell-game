...
export default function UserAvatar({ size = 40, avatarData }) {
  ...
  if (!data) {
    try {
      data = JSON.parse(localStorage.getItem("playerAvatar"));
    } catch {
      data = null;
    }
  }
  if (!data) {
    return <svg width={size} height={size} viewBox="0 0 80 100"><ellipse cx="40" cy="40" rx="30" ry="35" fill="#ccc" /></svg>;
  }
  const { skinColor, hairColor, hairstyle, accessory, eyeColor, shirtColor } = data;
  return (
    <svg width={size} height={size * 1.25} viewBox="0 0 80 110">
      ...
      {/* Eyes */}
      <ellipse cx="30" cy="45" rx="4" ry="3" fill={eyeColor || '#222'} />
      <ellipse cx="50" cy="45" rx="4" ry="3" fill={eyeColor || '#222'} />
      {/* Nose */}
      ...
      {/* T-shirt */}
      <rect x="15" y="75" width="50" height="30" rx="12" fill={shirtColor || '#ccc'} />
    </svg>
  );
}
